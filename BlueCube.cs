...
public class BlueCube : MonoBehaviour {
	...
	void Update () {
        ...
        if (Input.GetKeyDown(KeyCode.DownArrow) && gameObject.transform.position.x < 9)
        {
            
            transform.position= new Vector3(gameObject.transform.position.x+1, 0, gameObject.transform.position.z);
        }
        if (Input.GetKeyDown(KeyCode.UpArrow) && gameObject.transform.position.x > 0)
        {
            
            transform.position= new Vector3(gameObject.transform.position.x-1, 0, gameObject.transform.position.z);
        }
        if (Input.GetKeyDown(KeyCode.LeftArrow) && gameObject.transform.position.z > -1)
        {
            
            transform.position= new Vector3(gameObject.transform.position.x, 0, gameObject.transform.position.z-1);
        }
        if (Input.GetKeyDown(KeyCode.RightArrow) && gameObject.transform.position.z < 8)
        {
    ...
    private GameObject createBomb()
    {
        ...
        go.name = "BlueBomb";
        go.GetComponent<Renderer>().material.color = new Color(0.0f,0.0f,1.0f);
        return go;
    }
    private void OnGUI()
    {
    ...
    {
        if (other.gameObject.name == "RedBomb")
        {
